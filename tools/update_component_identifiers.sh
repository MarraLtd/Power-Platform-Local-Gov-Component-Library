...
do
    case "${flag}" in
        i) INPUT_SOLUTION_ZIP=${OPTARG};;
        o) OUTPUT_SOLUTION_ZIP=${OPTARG};;
    esac
done

if [ -z "${INPUT_SOLUTION_ZIP}" ]; then
...
if [[ "$INPUT_SOLUTION_ZIP" == "$OUTPUT_SOLUTION_ZIP" ]]; then
    echo "input zip (-i) and output zip (-o) values must be different"
fi

NEW_SOLUTION_DIR=$(dirname $INPUT_SOLUTION_ZIP)
...
pac solution unpack --zipfile $INPUT_SOLUTION_ZIP --folder $SOLUTION_UNPACKED_FOLDER --packagetype Managed
...
pac solution pack --zipfile $OUTPUT_SOLUTION_ZIP --folder $SOLUTION_UNPACKED_FOLDER --packagetype Managed

echo "Deleting unpacked solution folder"
rm -r $SOLUTION_UNPACKED_FOLDER

echo "Script ran successfully, output solution is available at ${OUTPUT_SOLUTION_ZIP}"